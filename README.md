...
- **SQL AST Inspection & Autonomous Rewriting (`sqlglot`)**: Blocks destructive verbs (`DROP`, `DELETE`, `TRUNCATE`), restricts sensitive tables (`credentials`, `salary_records`), and autonomously injects safety limits (`LIMIT 100`) and tenant isolation clauses (`WHERE tenant_id = '...'`) on `SELECT`/`UPDATE`/`DELETE`. For `INSERT`, tenant isolation is only enforced when the statement uses an explicit column list (`INSERT INTO t (tenant_id, ...) VALUES (...)`): a mismatched `tenant_id` value is denied, and a missing one is auto-injected. Positional `INSERT ... VALUES (...)` (no column list) and `INSERT ... SELECT` are not currently introspected and pass through unmodified — this requires table-schema knowledge the rewriter doesn't have.
